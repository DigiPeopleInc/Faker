...
class Image extends AbstractModule
{
    ...
    public function imageJpegByTheme(int $width = 1280, int $height = 720, string $theme = "all"): string
    {
        ...
        if ($width > Config::LOREMFLICKR_PICTURE_MAX_WIDTH || $height > Config::LOREMFLICKR_PICTURE_MAX_HEIGHT) {
            throw new Exception("Размер изображения должен быть не более ".Config::LOREMFLICKR_PICTURE_MAX_WIDTH."x".Config::LOREMFLICKR_PICTURE_MAX_HEIGHT);
        }
     ...
     * @throws Exception
     */
    public function image(int $width = 512, int $height = 512, string $backgroundColor = "808080", string $extension = "jpg", string $text = "", string $textColor = "000000"): string
    {
        ...
        if ($width > Config::PICTURE_MAX_WIDTH || $height > Config::PICTURE_MAX_HEIGHT) {
            throw new Exception("Размер изображения должен быть не более ".Config::PICTURE_MAX_WIDTH."x".Config::PICTURE_MAX_HEIGHT);
        }
        if (!in_array($extension, $allowedExtensions)) {
            throw new Exception("Расширение изображения не поддерживается, доступные расширения: ".implode(", ", $allowedExtensions));
        }
